...
Invalidation is primarily external — the data pipeline calls the refresh
endpoint in server.py after loading new data. The TTL is a safety ceiling
so a broken pipeline can't serve stale data forever.
"""
...
class PersistentQueryCache:
    ...
    # ---- index (hash -> {query, written_at, rows}) ---------------------------
    ...
    def get(self, query_text: str) -> Optional[pd.DataFrame]:
        ...
        # Read parquet outside the lock; then populate memory.
        try:
    ...
    def set(self, query_text: str, df: pd.DataFrame) -> None:
        ...
        # Write parquet outside the lock.
        try:
